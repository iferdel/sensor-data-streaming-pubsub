...
* `sensor-logs-ingester`: it consumes the sensor logs and saves them into a .log file to further processing in a centralized manner.
* `sensor-measurements-ingester`: it consumes the sensor(s) measurements and insert them into the postgres/timescaledb instance.
